...
function Activities({ navigation, route }) {
  ...
  const combineData = (names, minutes) => {
    return names.map((name, index) => {
      return { name: name, minutes: minutes[index] };
    });
  };
  ...
  useEffect(() => {
    ...
    const combinedData = combineData(names, mins);
  ...
  return (
    <ImageBackground style={styles.background} source={require("../assets/backgrounds/18_Max'sActivities.png")}>
      <View style = {{top: 55, right: 50}}>

        <Text style={{fontFamily: "BubbleBobble", fontSize: 35, color: "#00000", right: -170, bottom: 400, position: "absolute"}}>
          {route.params.name}'s Activities
        </Text>
      </View>
      <View style={{top: 100}}>
        {empty ? ( 
          <View style= {{alignItems: "center", bottom: 140}}>
          <Text style={{fontFamily: "BubbleBobble", fontSize: 30, color: "#00000"}}>
      ...
      <View style = {{position: "absolute", top: 800}}>
        <BlankButton 
